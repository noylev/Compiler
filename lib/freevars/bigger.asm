...
L_Bigger_Body:
	...
	CMP(FPARG(1),IMM(2));
	JUMP_LT(L_Bigger_Body_fewer_then_2);
...
L_Bigger_Body_loop:          
	...
	CMP(R1,R2);
	JUMP_EQ(L_Bigger_Body_loop_exit);
	...
	MOV(R8,INDD(FPARG(R1),2));
        ...
        CMP(R5,R7);
        JUMP_LE(L_Bigger_False);
        
        MOV(R5,INDD(FPARG(R1),1));
	MOV(R6,INDD(FPARG(R1),2));
	INCR(R1);
	JUMP(L_Bigger_Body_loop);   

L_Bigger_Body_loop_exit:
...
L_Bigger_Body_fewer_then_2:
        JUMP(L_Bigger_Body_loop_exit);
...
L_Bigger_Clos:
    ...
    MOV(INDD(R0,3),LABEL(L_Bigger_Body));
