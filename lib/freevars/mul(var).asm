...
L_Mul_Body:
	...
	CMP(FPARG(1),IMM(2));
	JUMP_LT(L_Mul_Body_fewer_then_2);
...
L_Mul_Body_loop:          
	...
	MOV(R6,INDD(R0,2));
	INCR(R1);
	JUMP(L_Mul_Body_loop);   
L_Mul_Body_loop_exit:
...
L_Mul_Body_fewer_then_2:
        CMP(FPARG(1),IMM(1));
        JUMP_LT(L_Mul_Zero_Item);
...
L_Mul_Clos:
    ...
    MOV(INDD(R0,3),LABEL(L_Mul_Body));
